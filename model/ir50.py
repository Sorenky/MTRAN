...
class Backbone_2(Module):
    def __init__(self, num_layers, drop_ratio, mode='ir'):
        super(Backbone_2, self).__init__()
        # assert num_layers in [50, 100, 152], 'num_layers should be 50,100, or 152'
        assert mode in ['ir', 'ir_se'], 'mode should be ir or ir_se'
        blocks2 = get_blocks(num_layers)[1]
        # blocks2 = get_blocks(num_layers)
        if mode == 'ir':
            unit_module = bottleneck_IR
        elif mode == 'ir_se':
...
class Backbone_3(Module):
    def __init__(self, num_layers, drop_ratio, mode='ir'):
        super(Backbone_3, self).__init__()
        # assert num_layers in [50, 100, 152], 'num_layers should be 50,100, or 152'
        assert mode in ['ir', 'ir_se'], 'mode should be ir or ir_se'
        blocks3 = get_blocks(num_layers)[2]
        if mode == 'ir':
            unit_module = bottleneck_IR
        elif mode == 'ir_se':
            unit_module = bottleneck_IR_SE
...
def get_ir50(num_layers=50, drop_ratio=0.0, mode='ir'):
    return Backbone(num_layers, drop_ratio, mode)

if __name__ == '__main__':
    model = Backbone(num_layers=50, drop_ratio=0.0, mode='ir')
